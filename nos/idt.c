...
void init_idt()
{
  ...
  memset((void *)&idt_entries, (sizeof(struct idt_entry_s)) * 256, 0);
  memset((void *)&interrupt_handlers, (sizeof(isr_t)) * 256, 0);
   ...
   * conflict with the interrupt numbers used by the CPU to signal exceptions
   * and faults. The normal thing to do is to remap the PICs so that IRQs 0..15
   * correspond to ISRs 32..47 (31 being the last CPU-used ISR). */
