...
void tty_scroll_line()
{
  ...
  for (i = 1; i < (TTY_CHAR_HIEGHT - 1); i++) {
    for (j = 0; j < TTY_CHAR_WIDTH; j++) {
      video_memory[((i - 1) * TTY_CHAR_WIDTH) + j] = video_memory[(i * TTY_CHAR_WIDTH) + j];
    }
  }
  ...
  for (i = 0; i < TTY_CHAR_WIDTH; i++) {
    video_memory[(TTY_CHAR_WIDTH * (TTY_CHAR_HIEGHT - 1)) + i] =                \
      low_nibble(video_memory[(TTY_CHAR_WIDTH * (TTY_CHAR_HIEGHT - 2)) + i]);
  }
...
void tty_putc(char c)
{
  ...
  if (c == ASCII_BACKSPACE && tty_cursor_x) {
    /* In case of backspace character, get the location of the last character
     * and mask off the low nibble. */
    location = video_memory + (tty_cursor_y * TTY_CHAR_WIDTH + tty_cursor_x - 1);
    *location = *location & 0xF;
    ...
    location = video_memory + (tty_cursor_y * TTY_CHAR_WIDTH + tty_cursor_x);
    *location = character;
    tty_cursor_x++;
  }
