                              ...
                              uint32_t *physical_address)
{
  ...
  if (physical_address != 0) {
    struct page_s *page = get_page((uint32_t)address, 0, kernel_directory);

    *physical_address = (page->frame * PAGE_SIZE) + ((uint32_t)address & 0xFFF);
  }
...
static void _heap_expand(struct heap_s *heap, uint32_t new_size)
{
  ...
  /* Return nothing if we attempt to expand heap larger than max address. */
                                     ...
                                     uint8_t page_align)
{
  ...
  while (i < heap->index.size) {
    ...
    if (page_align) {
      ...
      /* Page align the starting point of the header. Note that when a user
       * requests that memory be page-aligned, that request applies only to
       * memory that is user accessible. That means that the header address will
       * actually not be page-aligned. The address that we want to fall on a
       * boundary is the block location offset by the size of the header. */
      if (!is_page_aligned(location + HEADER_SIZE)) {
        offset = PAGE_SIZE - ((location + HEADER_SIZE) % PAGE_SIZE);
      }
                           ...
                           uint8_t read_only)
{
  ...
  /* Shift the start address forward to where we can start putting data. */
...
void *heap_alloc(struct heap_s *heap, uint32_t size, uint8_t page_align)
{
  ...
  if (i == -1) {
    ...
    while ((uint32_t)i < heap->index.size) {
      uint32_t temp = (uint32_t)ordered_array_lookup_index(&heap->index, i);
    ...
    if (index == (uint32_t)-1) {
      ...
      footer = (struct footer_s *)(old_end_address + header->size
                                            - FOOTER_SIZE);
      footer->id = FOOTER_ID;
      ...
      /* Re-write the footer. */
      footer = (struct footer_s*)((uint32_t)header + header->size
                                           - FOOTER_SIZE);
      footer->id = FOOTER_ID;
      footer->header = header;
    }
  ...
  /* We must now decide whether to split the hole we found into two parts. Is
   * the original hole size minus requested hole size less than the overhead for
   * adding a new hole? */
  ...
  if ((original_hole_size - total_size) < BLOCK_OVERHEAD) {
    /* Increae the requested size to the size of the hole we found. */
  ...
  if (page_align && (!is_page_aligned(original_hole_position))) {
    ...
    new_location = (original_hole_position + PAGE_SIZE - HEADER_SIZE
                    - (original_hole_position & 0xFFF));

    hole_header = (struct header_s*)original_hole_position;
    hole_header->size = (PAGE_SIZE - (original_hole_position & 0xFFF)
                         - HEADER_SIZE);
    ...
    /* We don't need this hole anymore, so remove it from the index. */
  ...
  block_footer = (struct footer_s*)(original_hole_position + HEADER_SIZE
                                    + size);
  ...
  if ((original_hole_size - total_size) > 0) {
    ...
    hole_footer = (struct footer_s*)((uint32_t)hole_header - total_size
                                     + original_hole_size - FOOTER_SIZE);
...
void heap_free(struct heap_s *heap, void *block)
{
  ...
  footer = (struct footer_s*)((uint32_t)header + header->size - FOOTER_SIZE);
  ...
  /* Now come decision logic as to whether to index this as a free hole. */
  test_footer = (struct footer_s*)((uint32_t)header - FOOTER_SIZE);

  /* If the memory immediately to the left of this block is a footer then we can
   * merge left. */
  if (is_footer(test_footer) && is_hole(test_footer->header)) {
    uint32_t cache_size = header->size;

    /* Cache our current size, and proceed to re-write the current header with
     * the new one, re-write the footer to point to the new header, update the
     * header size, and set the flag so that we don't add to index again. */
  ...
  /* If the memory immediately to the right of this block is a footer then we can
   * merge right. */
  ...
  if (test_header->id == HEADER_ID
    ...
    /* Increase the size the current header, and re-write the footer of the test
     * block to point to the current header. */
    header->size += test_header->size;
    test_footer = (struct footer_s*)((uint32_t)test_header + test_header->size
                                     - FOOTER_SIZE);
    footer = test_footer;

    /* Find and remove this header from the index. */
    while ((i < heap->index.size)
           && (ordered_array_lookup_index(&heap->index, i) != (void*)test_header)) {
      i++;
    }
  ...
  if ((uint32_t)footer + FOOTER_SIZE == heap->end_address) {
    ...
    uint32_t new_length = _heap_contract(heap, (uint32_t)header - heap->start_address);
    ...
    if (header->size - (old_length - new_length) > 0) {
      ...
      /* The block will no longer exist, so RIP block, and remove from index. */
      while ((i < heap->index.size)
      ...
      /* If we didn't find the block, there is nothing to remove. */
      if (i < heap->index.size) {
        ordered_array_remove_index(&heap->index, i);
      }
    }
