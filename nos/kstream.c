...
static void _set_kstream_style(enum kstream_style_e style)
{
	switch (style) {
	case KSTREAM_NORMAL:
		tty_set_colour(KSTREAM_NORMAL_BACKGROUND,
			       KSTREAM_NORMAL_FOREGROUND);
		break;
	case KSTREAM_HIGHLIGHT:
		tty_set_colour(KSTREAM_HIGHLIGHT_BACKGROUND,
			       KSTREAM_HIGHLIGHT_FOREGROUND);
		break;
	case KSTREAM_WARNING:
		tty_set_colour(KSTREAM_WARNING_BACKGROUND,
			       KSTREAM_WARNING_FOREGROUND);
		break;
	case KSTREAM_CRITICAL:
		tty_set_colour(KSTREAM_CRITICAL_BACKGROUND,
			       KSTREAM_CRITICAL_FOREGROUND);
		break;
	}
}
...
int kstream_message(enum kstream_message_e type, const char *format, ...)
{
	...
	if (tty_cursor_x) {
		/* If we're not at the start of a line, then scroll to the next
		 * one. This way, kstream messages always begin at the start of
		 * a line. */
...
int kstream_vprintf(enum kstream_style_e style,
		    const char *format,
		    va_list arguments)
{
	uint32_t i;
	...
	for (i = 0; i < format_length; i++) {
		...
		if (format[i] == '%') {
			if ((i + 1) < format_length && format[i+1] != '%') {
				...
				if (format[i+1] == 'p') {
					uint32_t pointer = va_arg(arguments,
								  uint32_t);
