...
void irq_handler(struct registers_s registers)
{
  ...
  /* If the interrupt originated form the slave PIC, then an EOI signal must be
   * sent to the slave PIC. */
  if (registers.interrupt_number >= 40) {
    PIC_SLAVE_COMMAND_OUT(0x20);
  }

  /* Send an EOI signal to the master PIC. N.b. this must be sent even if the
   * interrupt originated from the slave PIC, due to the manner in which they
   * are daisy-chained. */
...
void register_interrupt_handler(uint8_t interrupt_number, isr_t handler) {
  k_debug("Register ISR: [%d, %p]", interrupt_number, (uint32_t) handler);
