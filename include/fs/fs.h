...
/* These functions are used in struct fs_node as driver-dependent callbacks
...
struct fs_node {
	...
	struct fs_node *pointer;   /* Used by mountpoints and symlinks.       */
};
...
uint32_t fs_read(struct fs_node *node,
		 uint32_t offset, uint32_t size,
		 uint8_t *buffer);

uint32_t fd_write(struct fs_node *node,
		  uint32_t offset, uint32_t size,
		  uint8_t *buffer);
